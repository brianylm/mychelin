...
export function RefinementPanel({
  ...
  const generateSuggestions = async () => {
    setLoading(true);
    try {
      // Use AI to generate refinement suggestions based on closeness notes
...
${version.ingredients
  .map(
    (i: any) =>
      `- ${i.name}: ${i.quantity ?? ""} ${i.unit ?? ""} ${i.notes ? `(${i.notes})` : ""}`
  )
  .join("\n")}
...
Return ONLY the JSON array, no other text.`,
      ...
      if (res.ok) {
        ...
        // Try to parse AI response as suggestions
        try {
          const parsed =
          ...
          if (Array.isArray(parsed)) {
            ...
            setSelectedSuggestions(
              new Set(parsed.slice(0, 5).map((_: any, i: number) => i))
            );
          }
        } catch {
          // Fallback: generate basic suggestions from closeness notes
  ...
  const generateFallbackSuggestions = () => {
    // Generate simple suggestions based on common patterns in closeness notes
    const notes = (version.closenessNotes ?? "").toLowerCase();
    const fallback: Suggestion[] = [];

    if (notes.includes("salty") || notes.includes("salt")) {
      const saltIng = version.ingredients.find(
        (i: any) =>
          i.name.toLowerCase().includes("salt") ||
          i.name.toLowerCase().includes("soy sauce") ||
          i.name.toLowerCase().includes("fish sauce")
      );
      if (saltIng) {
        fallback.push({
    ...
    if (notes.includes("sweet") || notes.includes("sugar")) {
      const sugarIng = version.ingredients.find(
        (i: any) =>
  ...
  const handleApply = async () => {
    setSaving(true);
    try {
      // Build refined ingredients based on selected suggestions
      const refinedIngredients = version.ingredients.map((ing: any) => {
        const suggestion = suggestions.find(
          (s, i) =>
            selectedSuggestions.has(i) &&
            s.ingredient.toLowerCase() === ing.name.toLowerCase()
        );
        if (suggestion) {
          // Parse the suggested amount
          const match = suggestion.suggestedAmount.match(
            /^([\d.]+)\s*(.*)$/
          );
          if (match) {
            return {
              ...
              unit: match[2] || ing.unit,
              notes: `${ing.notes ? ing.notes + " | " : ""}Refined: ${suggestion.reason}`,
            };
          }
        }
        return ing;
      ...
      const res = await fetch(`/api/recipes/${recipeId}/versions`, {
        ...
        body: JSON.stringify({
          ...
          captureMethod: "refinement",
          ingredients: refinedIngredients,
          instructions: version.instructions,
          notes: version.notes,
          changeNote: `AI refinement based on v${version.versionNumber} cook-along feedback`,
          setActive: false,
        }),
      });
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ y: 40, opacity: 0 }}
        className="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white sm:rounded-2xl"
          ...
          {loading ? (
            ...
            <div className="py-8 text-center">
              <p className="text-sm text-neutral-500">
                No specific suggestions could be generated.
              </p>
              <p className="mt-1 text-xs text-neutral-400">
                Try adding more detailed notes about what was different.
              </p>
            </div>
          ) : (
        ...
        {suggestions.length > 0 && (
          <div className="sticky bottom-0 flex items-center justify-between border-t border-neutral-100 bg-white px-4 py-3">
            ...
            >
              ...
              Apply {selectedSuggestions.size} Suggestion
