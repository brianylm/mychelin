...
import { X, ArrowLeftRight, Star, Plus, Minus, RefreshCw } from "lucide-react";
...
export function VersionCompare({
  ...
  const [tab, setTab] = useState<"ingredients" | "instructions" | "notes">(
    "ingredients"
  );
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/recipes/${recipeId}/versions/compare?base=${baseVersionId}&compare=${compareVersionId}`
        );
        if (res.ok) {
          ...
          {loading ? (
            ...
            <div>
              ...
              <div className="flex border-b border-neutral-100">
                {(["ingredients", "instructions", "notes"] as const).map(
                  (t) => (
                    <button
                      key={t}
                      onClick={() => setTab(t)}
                    ...
                    {data.ingredientDiffs.length === 0 ? (
                      ...
                      data.ingredientDiffs.map((diff, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`flex items-center gap-2 rounded-xl px-3 py-2 ${style.bg}`}
                          >
                            {Icon && (
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {diff.status === "changed" && (
                                <div className="mt-0.5 flex gap-3 text-[11px]">
                                  <span className="text-neutral-400">
                                    {diff.base?.quantity} {diff.base?.unit} →{" "}
                                    {diff.compare?.quantity}{" "}
                                    {diff.compare?.unit}
                                  </span>
                              ...
                              {diff.status === "removed" && diff.base && (
                                <div className="mt-0.5 text-[11px] text-neutral-400">
                                  Was: {diff.base.quantity} {diff.base.unit}
                                </div>
                              )}
                              {diff.status === "added" && diff.compare && (
                                <div className="mt-0.5 text-[11px] text-neutral-400">
                                  {diff.compare.quantity} {diff.compare.unit}
                                </div>
                              )}
                            </div>
                          </div>
                ...
                {tab === "instructions" && (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      {data.baseInstructions.length === 0 ? (
                        <p className="text-xs text-neutral-400 italic">
                          No instructions
                        </p>
                      ) : (
                        <ol className="space-y-2">
                          {data.baseInstructions.map(
                            (inst: any, i: number) => (
                              <li
                                key={i}
                    ...
                    <div>
                      ...
                      {data.compareInstructions.length === 0 ? (
                        <p className="text-xs text-neutral-400 italic">
                          No instructions
                        </p>
                      ) : (
                        <ol className="space-y-2">
                          {data.compareInstructions.map(
                            (inst: any, i: number) => (
                              <li
                                key={i}
                                className="text-xs leading-relaxed text-neutral-700"
              ...
              {data.ingredientDiffs.some((d) => d.status !== "unchanged") && (
                <div className="border-t border-neutral-100 px-4 py-3">
                  <div className="flex flex-wrap gap-2 text-[11px]">
                    {(() => {
                      const added = data.ingredientDiffs.filter(
                        (d) => d.status === "added"
                      ).length;
                      const removed = data.ingredientDiffs.filter(
                        (d) => d.status === "removed"
                      ).length;
                      const changed = data.ingredientDiffs.filter(
                        (d) => d.status === "changed"
                      ).length;
                      return (
                        <>
