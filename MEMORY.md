...
- Same session: card now always ends with a synthetic **"Combine all"** column (merge icon, not a numbered step) whose block spans every ingredient row — skipped when the last real step already encompasses all ingredients or there are no ingredient rows. Card-only visual; excluded from the markdown copy (`recipeToMarkdown` filters `combine`). Covered in `cooking-card-layout.test.ts` + `cooking-card-markdown.test.ts`.
