...
- `npm run smoke:privacy` could not reach its assertions locally because the documented Edge-runtime `bcryptjs` `setImmediate` limitation makes local email signup return 500. No auth/API code changed in this uplift; production privacy and pilot smoke remain the post-deploy gate.
- Follow-ups: pilot-test authenticated workflows in production, consider a centralized focus-trapped Dialog/WorkflowDialog only after pilot feedback, and measure Radix Themes public-page bundle cost before changing runtime dependencies.
