...
## 2026-07-19 - Full Mychelin kitchen workbench UI uplift

- Established commit `7465297` (`chore: checkpoint current Mychelin product state`) as the pre-uplift rollback point.
- Extended the Hallmark/Agrippa design-system work across the active product: shared Button/PageHeader/SectionHeader/Skeleton/CollapsibleSection/FilterBar patterns; solid global header; persistent desktop sidebar; common mobile drawer and five-part bottom navigation; one-tap Create recipe routes; and semantic canvas, focus, reduced-motion, and zero-letter-spacing rules.
- Reworked Library and Books into scannable record rows with title/ingredient/cuisine/note search, visible filters/sorts, flags, last-cooked evidence, and calmer recipe reading/editing hierarchy.
- Flattened capture/import/conversation, Cook with me, multi-dish timers, attempts/versions, Activity, Planner, Shopping, Fridge, onboarding, Profile, pilot feedback, and Admin Analytics into divider-led operational layouts with kitchen-friendly controls.
- Standardized active auth/reset, search, sharing, calendar, book, Add to book, Log cook, version comparison/details, and suggested-refinement dialogs with solid surfaces, Lucide controls, 44px actions, labels, and visible focus. Existing permissions, autosave, capture, timer, and version behavior were preserved.
- Improved recipe editor ergonomics: 44px ingredient/step controls, Lucide drag handle, same-row desktop ingredient editing, clearer mobile measurement fields, and mobile-safe rating rows.
- Screenshot-driven landing corrections preserve both cooks at tablet width and keep the complete hero plus next-section hint visible on a 320x568 short phone. Service-worker cache advanced from `mychelin-v11` to `mychelin-v12`.
- Updated the in-app changelog, `app/UI-SYSTEM-AUDIT.md` implementation status, and ROADMAP mobile-PWA progress.
- Validation passed: full `npm run lint`, `npx tsc --noEmit`, final `npm run build`, `node --check public/sw.js`, and `git diff --check`.
- Temporary Playwright Chromium visual checks passed for landing at 320x568, 375x812, 414x896, 768x1024, and 1440x900, plus auth at 320x568, 375x812, 768x1024, and 1440x900. Sharp dimension/entropy checks confirmed every frame was nonblank. Authenticated synthetic-data checks also covered Library at 320x640, 375x812, 414x896, and 768x1024 plus recipe detail at 375x812 and 1440x1000; those captures caught and verified a tablet list breakpoint fix, removal of the duplicate Library FAB, and the shorter mobile Shop label. The synthetic QA account and cascaded records were removed afterward (0 remaining).
- `npm run smoke:privacy` could not reach its assertions locally because the documented Edge-runtime `bcryptjs` `setImmediate` limitation makes local email signup return 500. No auth/API code changed in this uplift.
...
## Slice D — Capture Flows

Added 2026-07-30. The three capture modals (PasteRecipeModal, AiDraftRecipeModal, ManualRecipeScratchpadModal) now sit on `WorkflowDialog` (`src/components/ui/WorkflowDialog.tsx`: Dialog + shared `WorkflowProcessing` / `WorkflowError` step bodies; error technical details are collapsed, copy is user-first). Content-loss guarantees: pasted/typed text is backed up to localStorage on every keystroke (`src/lib/capture-draft-storage.ts`) and restored on reopen; URL import that finds no recipe hands the scraped page text to Text mode instead of discarding it; a failed draft-save PATCH stays on the error step instead of close-and-delete. ConversationCapture remains paused/untouched. Acceptance: no pasted text or prompt is lost on extraction failure, save failure, or accidental close.
...
## Live Conversation — Dialect Fix, Card Lifecycle, Consent

Added 2026-07-30. Diagnosis: OpenAI Realtime transcription used to kill the dialect-strong Gemini chunk path when it connected, so Hokkien sessions were transcribed by the weakest model for the job — and OpenAI API has no free tier. Now: `CONVERSATION_REALTIME_ENABLED` (default false) gates OpenAI off; the Gemini chunk recorder always runs the full session and accumulates a canonical backup transcript (`backupSegmentsRef` + `conversation-transcript.ts`) used for naming/extraction, while live captions stay whatever's fastest. A final assist call on the canonical transcript fires at the naming step. Question cards: ≤3 visible + Show more + per-card dismiss (`question-cards.ts`); answered cards auto-resolve via the existing assist poll. Consent: first-recording-per-device warm checkpoint (`mychelin:voice-consent-v1`); no raw audio stored. Modal migrated to WorkflowDialog. Cost guardrails: OpenAI flag-off = $0 (user to hard-cap/delete the key in OpenAI Billing → Limits); Gemini stays free while the AI Studio project has no billing account.

## 2026-08-03 - First Recipe Guided Mission packet grilled to ready

- Grilled `docs/work-packets/first-recipe-guided-mission.md` with B; all open questions locked to ready-for-first-pass status.
- Decisions: (1) sample recipe = reuse the 3 existing starter recipes (no synthetic sandbox in v1); (2) entry = optional "Cook your first dish" dashboard card post-onboarding (onboarding untouched); (3) dismissible + resumable until the first attempt is recorded; (4) only a recipe is required before Cook With Me — plan/shopping steps are optional with skip; (5) completion screen celebrates + shows "1 of N meals this week" + next actions (save as version / plan next meal / done); (6) rhythm = surface existing attempt-based weekly progress only, no new tracking/schema.
...
## 2026-08-03 - Flag cleanup + difficulty rating removed

- Removed the "Try soon" recipe flag entirely: `RECIPE_FLAG_OPTIONS`, planner pick weighting (`planner-randomize.ts`), meal-plan sort priority (`MealPlanView.tsx`), recipe badge map (`RecipeView.tsx`), and its unit-test case. One-time cleanup deletes stale `try_soon` rows (`ensureTrySoonFlagCleanup` in `ensure-schema.ts`, wired into the flags read path). Only the "New" (`newly_added`) flag remains.
- "New" flag lifecycle: recipes get it by default on creation — manual create via `POST /api/recipes` already defaulted to `["newly_added"]`; fork and share-save now set it too. Logging any attempt (`POST /api/recipes/[id]/attempts`) clears the recipe's `newly_added` flag (idempotent, best-effort so a flags-table hiccup never fails the attempt save).
...
## 2026-08-03 - Cooking card ingredient↔step intelligence

- B rejected the synthetic "Combine all" column (a misread of "final combine step") — the real ask is **intelligent ingredient↔step matching** so the card reorders ingredients where they make sense and shows combination points even when a step never names its ingredients.
- `matchIngredientsForStep` (`src/lib/step-ingredient-amounts.ts`) now adds, on top of phrase/fuzzy matching: whole-dish phrases → every ingredient; ingredient-note hints (xiao bai cai noted "stems separated from leaves" now matches a step saying "add the stems"); category words with plural-only exact-token triggers ("the aromatics / greens / meat / sauces" resolve to the actual ingredients; ambiguous singular "the sauce" stays unmatched); unambiguous generic tokens ("oil" → the single cooking oil; several sauces → skipped); and frying implies the recipe's single oil. Default match limit raised 5 → 12.
- `ENCOMPASS_ALL_PATTERNS`/`stepEncompassesAll` moved to the matcher module (shared; re-exported by `cooking-card-layout.ts`); the "combine all <subset>" pattern was tightened so "Combine all the dry ingredients" does not fire whole-dish.
- Result for starter recipes: xiao bai cai rows read garlic | cooking oil | xiao bai cai | light soy | water (stems step claims the greens); onion omelette's oil is claimed at the "Fry onions" step. 98 unit tests pass.

## 2026-08-08 - Households Slice 1: membership + shared plan with per-member blocking

- New tables `households` (join_code unique, deleted_at reserved for the Slice 2 30-day flow), `household_members` (flat admin/member), `household_activity_log` (mirrors book_activity_log), `household_member_blocks` (scope slot/day/week/month; week rows store the Monday, month rows the 1st, meal_type '' for non-slot scopes so the unique index stays total); nullable `meal_plans.household_id` scopes the shared plan (NULL = personal, all existing rows untouched). Schema allows many households per user; UI/API gate to one (friendly 409).
- Migration `app/drizzle/0028_households.sql` is HAND-WRITTEN to match `ensureHouseholdTables()` in `ensure-schema.ts`: `npm run db:generate` diffs against a stale snapshot chain (meta only has snapshots through 0008 while SQL files run to 0027) and emits years of unrelated drift — generated output was discarded. Known repo issue worth fixing separately. Applied to the dev Turso DB via `app/scripts/apply-households-migration.mjs` (idempotent apply + verify).
- API: `/api/households` (GET mine+members+activity, POST create), `/join` (by 6-char code, case-insensitive), `/leave`, `/members` (PATCH promote, DELETE remove — flat admins: any admin can remove any admin incl. creator; self-removal goes through /leave). All Edge + hnd1, membership-scoped via `src/lib/households.ts`; routes 404 when `HOUSEHOLDS_ENABLED` is false.
- Meal plans: GET/POST `/api/meal-plans` and `[id]` routes branch on household membership — household users read/write household-scoped slots (userId = who added it, `addedByName` in responses); solo users see zero change. Household blocking never clears slot plans (unlike solo blocks); adding a meal lifts only the planner's own slot-scope block. Randomize endpoints unchanged — they ride the same routes.
- Blocking/servings math in pure libs `household-blocking.ts` + `household-portions.ts` (unit-tested): recipe yield is the base for the FULL household, scaled display-side by non-blocked/member share ("serves 3 of 4", "nobody eating" when fully blocked); stored quantities never rewritten.
- UI: Household tab (House icon, flag-gated in BottomNav/DesktopNav) + `household/HouseholdView.tsx` (create/join, member list with role badges, promote/remove/leave, activity feed). Planner shows "Out: {name}" chips (tap your own to unblock), "added by X" attribution, and scaled servings; the Ban button opens a 4-scope sheet in household mode. SW cache bumped to v12.
- Trap hit during e2e: a zombie `next dev` from Aug 05 was squatting on :3100 and playwright's `reuseExistingServer` kept reusing it — hot-reload served a stale/mixed module graph (GET /api/households returned null right after a 201 create). Kill stray dev servers before trusting local e2e.
- Validation: tsc, eslint (changed files), 122 vitest, production build, full Playwright suite, git diff --check all pass. `RefinementPanel.tsx` (`any` → typed) and `AddToBookModal.tsx` (unescaped quotes) carry small lint fixes: both files were already touched on this branch, so lint-changed.sh surfaces their pre-existing debt and the gate requires the fixes.
- Slice 2 (NOT built): shared inventory/shopping, recipe share-to-household, 30-day deletion + join-code reactivation, ghost copies, attempts visibility.

## 2026-08-08 - Households Slice 2: shared inventory, shared shopping list, 30-day deletion

- Schema (`app/drizzle/0029_households_slice2.sql`, HAND-WRITTEN like 0028 — `db:generate` still emits stale-snapshot drift; applied to dev Turso via `scripts/apply-households-slice2-migration.mjs`, runtime ensure `ensureHouseholdSlice2Tables()`): nullable `inventory.household_id`; new `shopping_list_items` (tick/move state + manual adds, unique household+item_key); ghost snapshot columns `meal_plans.ghost_title/ghost_yield/ghosted_at`. All additive; solo rows untouched. NOTE: there was NO pre-existing shopping-list table — the solo list is generated on read with component-local ticks; the new table only ever holds household rows in v1 (user_id column is the reserved seam for solo persistence later).
- Shared inventory: `/api/inventory` routes branch on `getUserHousehold` (shared rows vs `userId + household_id IS NULL`); any member can add/edit/remove; add/edit/remove are attributed in the activity feed (`added_item`/`edited_item`/`removed_item`). Personal pre-household rows stay personal and are simply not shown while in a household.
- Cook-time confirm-deduct: attempts POST now returns a `deduction` proposal when (and only when) the meal is on the cook's household plan AND the cook owns the recipe (trap check, enforced server-side again in the deduct route). `CookWithMeSession` shows a confirm sheet with editable quantities and "not tracked" list before closing. Deduction scales by slot servings × non-blocked-eater factor (`blockingScaleFactor`), matches shared rows by catalog id then case-insensitive name+unit, may drive stock negative (warning, never blocks). MultiCookWithMeSession batch flow intentionally shows no deduction sheet (punt).
- Shared shopping list: GET generates from the shared plan scaled by non-blocked eaters (fully blocked slots drop out), offsets shared inventory, merges manual adds, annotates server-persisted ticks (`tickedByName`), and returns `movedItems` (shown as moved, never re-pushed). New routes: `/api/shopping-list/tick` (step 1, never touches inventory), `/api/shopping-list/items` (manual add/remove), `/api/shopping-list/move-to-inventory` (step 2, edit-before-confirm overrides; idempotent via movedAt stamped in the same request — partitionForMove only ever returns moved_at NULL rows). Pure math in `household-shopping.ts` / `household-inventory.ts` (unit-tested).
- 30-day deletion flow: empty household (last leave/removal) or admin DELETE /api/households sets deleted_at; household is dead/recoverable — gone from all APIs except join. Join with the code inside 30 days reactivates wholesale (deleted_at cleared, reactivating user becomes admin); past the window the join route purges and 410s "permanently deleted". Lazy purge also runs on membership load (getUserHousehold) and in the daily notifications cron (`purgeExpiredHouseholds`). Purge deletes child rows explicitly — SQLite FK cascade enforcement is connection-dependent. Departure cleanup: member leave/removal now ghosts their on-plan recipes (title/yield snapshot) and deletes their household_member_blocks (Slice 1 punt).
- Share-link findings (verify-only task): `share_links` are user-owned (created_by → users, resourceType recipe|book) — NO household-owned share links exist, so the "will be deleted on {date}" tag was NOT built (nothing to attach it to). `/api/share/[token]/save` is recipe-only — books have no import-all equivalent (404s non-recipe links). Report to B.
- UI: "Pantry"→"Inventory" rename (FridgeView header/copy, nav labels; storage locations unchanged), zero/negative stock badge, manual-reconcile inline edit, shared-with-household badges; ShoppingListView household mode (server ticks, manual add, move-to-inventory sheet, moved section); HouseholdView admin delete button + new activity labels; planner falls back to ghost title/yield when the recipe join is gone. SW cache bumped to v13.
- e2e: `e2e/households-slice2.spec.ts` (two synthetic users — shared inventory add/edit + isolation, blocking-scaled shopping quantities, tick → move idempotency incl. double-push, ghost slot after leave + block cleanup, empty → reactivate as admin within window, time-travel 31d → 410 + purged). helpers.ts gained `devDb()` + `cleanupHouseholds()` (explicit child deletes, run before cleanupUsers).
- Validation: tsc, lint-changed (0 errors), 154 vitest (32 new), production build, full Playwright (15 pass / 1 pre-existing skip, chromium+mobile), git diff --check — all green. NOT deployed; prod Turso migration still needs applying at deploy time (runtime ensure covers preview/prod lazily).
- Slice 3 (NOT built): recipe share-to-household flag + bulk share, ephemeral cook/attempts-visibility relaxation, import flow for household recipes.
...
- Route smoke on the preview: `/api/inventory`, `/api/shopping-list`, `/api/households` → 401 (live); `/api/households/members`, `/api/households/join`, `/api/households/leave`, `/api/households/inventory/deduct` → 405 (POST/PATCH routes live). Migration `0029_households_slice2.sql` is applied lazily on preview/prod via `ensureHouseholdSlice2Tables()` — no manual step needed.
- Open items unchanged: Slice 3 (recipe share-to-household flag + bulk share, ephemeral Cook With Me + attempts visibility, import flow) still unbuilt; prod Turso migration still needs applying at deploy time; books have no import-all affordance (report to B — no household-owned share links exist to attach a deletion tag to).

## 2026-08-08 - Households Slice 3: recipe sharing, ephemeral cook-along, import

- Scope: shared recipes to a household, members cook along ephemerally, and members import a copy into their own library. Committed `b198452` on `ui-uplift`.
- Schema (`app/drizzle/0030_households_slice3.sql`, hand-written; applied to dev Turso via `scripts/apply-households-slice3-migration.mjs`, runtime ensure `ensureHouseholdSlice3Columns()`): nullable `recipes.shared_to_household_at` + `shared_to_household_by → users`. Additive; solo rows untouched.
- Access: new read-only predicate `canUserReadRecipe` = `canUserAccessRecipe OR canReadHouseholdSharedRecipe`. Deliberately NOT in `recipesVisibleTo` — shared recipes stay out of personal libraries and out of every mutation gate. Writes still go through `canUserAccessRecipe`/`canUserEditRecipe` unchanged (member PATCH to a shared recipe = 404).
- API: `POST /api/recipes/[id]/household-share` `{shared}` (owner-only toggle), `GET /api/households/recipes` (household shared list w/ ownerName + isOwner), `POST /api/households/recipes` (bulk share-all of owner's current active library — recipes created later stay private), `POST /api/households/recipes/[id]/import` (forks via new `src/lib/recipe-fork.ts`). Activity feed: `shared_recipe`/`unshared_recipe`/`shared_all_recipes`/`imported_recipe`.
- Fork: `forkRecipeToUser(currentUserId, sourceRecipeId)` copies LIVE rows (ingredients/instructions), creates v1 version, stamps `forked_from = "<id>:<title>"`, marks `newly_added`. Distinct from `/api/share/[token]/save` (definitive-snapshot); this one copies live untested rows.
- Ephemeral Cook With Me: `CookWithMeSession` gained `ephemeral?: boolean` prop; `RecipeWorkspace.startCookSession` computes it as `recipe.userId != null && recipe.userId !== user?.id`. Ephemeral sessions skip attempt/next-try/deduction persistence entirely, hide the next-try section, and show practice-mode copy. `attempts`/`next-try` GET routes relax visibility for household members reading a shared recipe (member sees all attempts; owner sees own) — read-only relaxation, POST still gates on ownership.
- UI: owner-only "Share with household" toggle in RecipeView's Share+Delete row; HouseholdView gained a Shared recipes card (read-only detail modal + Import button) and `onCookRecipe` wiring so members can cook a shared recipe in-place.
- **DB fix that unblocked everything**: `recipes.forked_from` had a spurious `REFERENCES recipes(id)` FK baked into BOTH Turso DBs (schema.ts + migration 0008 define it as plain text; the app writes `"<id>:<title>"` strings), so EVERY fork (share-save + slice3 import) failed with FK constraint. SQLite can't drop a column FK in place → rebuilt the table via `drizzle/0031_drop_recipes_forked_from_fk.sql` + `scripts/rebuild-recipes-drop-fork-fk.mjs`: `foreign_keys=OFF` → create `recipes_new` (identical minus FK) → copy rows → drop old → rename → restore `sqlite_sequence` (seq was 303, burned ids). **Learned**: the libsql remote client does NOT hold interactive `BEGIN`/`ROLLBACK` across `execute()` calls — each statement auto-commits, so the "dry-run" actually applied (verified clean anyway: 31 rows intact, 3 legit FKs remain, `foreign_key_check` = 0, sequence 303). Full pre-rebuild backup kept in `scripts/.backups/` (gitignored — contains account emails).
- Confirmed DB identity with B via sample rows: `.env.local` → `mychelin-brianylm` is the CORRECT dev/test DB (23 users, 31 recipes, current schema). `.env` → `mychelin-us-brianylm` is an old March scratch DB (1 user, 3 recipes, pre-multi-user — no `status` column). `.env.local` precedence in scripts is already correct.
- e2e: `e2e/households-slice3.spec.ts` — 4/4 passing (chromium + mobile): per-recipe share, read, write isolation (PATCH=404), import lineage (forked_from contains source id, own copy editable), activity feed, solo-user isolation, bulk share, post-share recipes stay private, unshare revokes reads but B's import survives. Fix along the way: share-all spec URL was `/api/households/recipes/share-all` but the route is `POST /api/households/recipes` (no `share-all` subroute) → updated spec. Cleanup verified (0 leftover e2e users/households).
- Validation: tsc, eslint (Slice 3 files, 0 errors), 154 vitest, production build, full e2e 4/4 — all green. Committed `b198452`.
- Deploy: preview `app-ae429k3hj-brianylms-projects.vercel.app` re-aliased to `https://mychelin-ui-uplift.vercel.app`. Preview is behind Vercel SSO (project-level; the app's own `/login` is also intercepted for unauthenticated curl) — consistent with the earlier note that "preview auth was not usable for user testing". B's logged-in browser passes straight through. Full behavior was already validated by the e2e suite against the same dev DB.
- Open items: prod Turso migration still needs applying at deploy time (runtime ensure covers preview/prod lazily); books have no import-all affordance; prod deploy still pending B's "test all 3 together" pass.

## 2026-08-08 - Playwright validation of ui-uplift features (cooking card, mission, planner)

- Baseline full suite first: **19 passed / 1 skipped** (planner-nav drawer test is mobile-only, skips on chromium) across 6 specs × chromium+mobile — confirms the 0031 FK rebuild did not regress households/slice2/slice3.
- Three new specs, committed `2a6d278`, all green across chromium + mobile:
  - `e2e/cooking-card-depth.spec.ts` (3): ingredient rail reorders into per-step bands (step-first reference wins — garlic listed after rice in the recipe but referenced in step 1 must come before rice in the rail); heat chip from `[heat:medium]` tip prefix + timer chip from explicit "20 min" (asserted with exact regexes `/^Medium$/`, `/^20 min$/` so the instruction sentence can't false-positive the substring); Start Cooking → CookWithMeSession shows step 1 + "Next step".
  - `e2e/first-cook-mission.spec.ts` (2): mission card → "Cook with me" → pick dish → Continue → "Plan it"; dismiss keeps it hidden this session. **Lesson**: the mission recipe-title assertion must be dialog-scoped (`getByRole("dialog")`) — the RecipeView card grid behind the modal renders the same title and trips strict mode.
  - `e2e/planner-randomize.spec.ts` (2): Randomize week → review dialog → roll → "Planned N meals for this week" banner → Undo clears; block-a-meal empties a slot. **Lessons**: the roll fills every open week slot with serial POSTs (~1s each on remote dev Turso) — banner assertion needs a 60s timeout; `getByRole({ name: "Undo" })` is substring-matched and also catches "Dismiss undo banner" → must use `{ exact: true }`.
- App bugs surfaced by validation (pre-existing, NOT test bugs, NOT yet fixed):
  1. `POST /api/meal-plans` intermittently 500s with `TypeError: Value is not JSON serializable` — the post-insert `findFirst` re-fetch can come back undefined, and `NextResponse.json(undefined)` throws in the edge runtime. Direct probe (insert + immediate refetch ×5) showed 0/5 misses for a normal insert, so it's context-dependent, not a general replication race.
